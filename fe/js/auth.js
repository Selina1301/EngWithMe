function initAuthForms() {
    // Dynamically resolve Google OAuth login URL
    document.querySelectorAll('a[href*="google_login.php"]').forEach((a) => {
      a.href = resolveApiUrl("auth/google_login.php");
    });
  ...
  registerForm?.addEventListener("submit", async (event) => {
    ...
    await submitAuthForm(registerForm, resolveApiUrl("auth/register.php"));
  });

  loginForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    await submitAuthForm(loginForm, resolveApiUrl("auth/login.php"));
  });
  ...
  if (loginForm) {
    ...
    // Tự động điền Email đã ghi nhớ và tích sẵn checkbox "Ghi nhớ" nếu đã lưu trước đó
    const rememberCheckbox = loginForm.querySelector('input[name="remember"]');
    const emailInput = loginForm.querySelector('input[name="email"]');
    const rememberedEmail = localStorage.getItem("engWithMeRememberedEmail");
  ...
  resetForm?.addEventListener("submit", async (event) => {
    event.preventDefault();
    await submitAuthForm(resetForm, resolveApiUrl("auth/reset_password.php"));
  });
}
...
async function submitAuthForm(form, endpoint) {
  ...
  try {
    ...
    if (result.requires_otp) {
      ...
      openOtpModal(result.email || form.elements.email?.value || "", form, isLoginOtp, result.message);
      return;
    }

    if (result.user) {
      // Xử lý lưu hoặc xóa Email ghi nhớ dựa trên trạng thái Checkbox "Ghi nhớ"
      const rememberCheckbox = form.querySelector('input[name="remember"]');
      const emailInput = form.querySelector('input[name="email"]');
      if (emailInput && emailInput.value) {
        if (rememberCheckbox && rememberCheckbox.checked) {
          localStorage.setItem("engWithMeRememberedEmail", emailInput.value.trim());
        } else {
...
function openOtpModal(email, originalForm, isLoginOtp = false, initialMsg = "") {
  const verifyEndpoint = resolveApiUrl("auth/verify_otp.php");
  const resendEndpoint = isLoginOtp ? resolveApiUrl("auth/login.php") : resolveApiUrl("auth/register.php");
  ...
  if (errEl) {
    errEl.textContent = initialMsg || `Vui lòng kiểm tra hòm thư Gmail ${email} để lấy mã OTP 6 số.`;
  ...
  resendBtn.onclick = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        if (errEl) {
          errEl.textContent = res.message || "Mã OTP mới đã được gửi thành công tới Gmail!";
          errEl.style.color = "var(--success)";
        }
