(function () {
  ...
  function renderReadingLessons() {
    ...
    topicList.innerHTML = sortedLessons
      .map((lesson, index) => {
        ...
        const isHardLocked = lesson.level === "hard" && !isVip;
        return `
        <article class="reading-topic-card${index === 0 ? " is-active" : ""}${isViewed ? " is-viewed" : ""}${isHardLocked ? " is-vip-locked" : ""}" data-reading-topic-link="${safeText(lesson.id)}" data-reading-level="${safeText(lesson.level)}" data-original-index="${index}">
          <button class="reading-topic-open" type="button" data-reading-topic-open aria-label="Mở bài ${safeText(lesson.title)}"></button>
          <span>${safeText(lesson.level.charAt(0).toUpperCase() + lesson.level.slice(1))} ${isHardLocked ? '<strong style="color: #f87171; margin-left: 4px;">🔒 PRO/PREMIUM</strong>' : ""}</span>
          <strong>${safeText(lesson.title)} ${isHardLocked ? '<i class="ti-lock" style="color: #f87171; font-size: 0.9rem; margin-left: 4px;"></i>' : ""}</strong>
          <small>${lesson.lines.length} dòng đọc</small>
          <i class="topic-check ${isHardLocked ? "ti-lock" : "ti-check"}" aria-hidden="true" style="${isHardLocked ? "color: #f87171;" : ""}"></i>
        </article>
      `;
      })
      .join("");
  ...
  function bindEvents() {
    ...
    // Level Tab Switching
    const updateReadingLevelTabs = () => {
      const isVip = typeof isUserVip === "function" && isUserVip();
      readingLevelTabs.forEach((tab) => {
        const lvl = tab.dataset.readingLevelTab;
        if (lvl === "hard") {
          tab.innerHTML = !isVip ? 'Hard <i class="ti-lock" style="color: #f87171; margin-left: 3px;"></i>' : 'Hard';
          tab.title = !isVip ? "Khóa chế độ Khó (Hard Mode) - Cần nâng cấp Pro / Premium" : "";
        }
        tab.classList.toggle("active", lvl === activeReadingLevel);
      });
    };
