(function () {
  ...
  function loadFallbackSync() {
    if (typeof window !== "undefined") {
      ...
      const rawPrimary = rawSource.map((spec, index) => createMissionFromSpec(spec, spec.sessionOrder || (index % 6) + 1));
      const seenIds = new Set();
      
      const groupedByGoal = {};
      rawPrimary.forEach((spec) => {
        if (!spec || !spec.id || seenIds.has(spec.id)) return;
        seenIds.add(spec.id);
        const goalKey = spec.goal || defaultGoal;
        if (!groupedByGoal[goalKey]) groupedByGoal[goalKey] = [];
        if (groupedByGoal[goalKey].length < 6) {
          groupedByGoal[goalKey].push(spec);
        }
      });

      const finalMissions = [];
      const knownGoalKeys = Object.keys(goalText);
      const allGoalKeys = Array.from(new Set([...knownGoalKeys, ...Object.keys(groupedByGoal)]));

      allGoalKeys.forEach((goalKey) => {
        const goalMissions = (groupedByGoal[goalKey] || []);
        goalMissions.sort((a, b) => (a.sessionOrder || 0) - (b.sessionOrder || 0));
        goalMissions.forEach((m, idx) => {
          m.sessionOrder = idx + 1;
          m.label = `Session ${idx + 1}`;
        });
        finalMissions.push(...goalMissions);
      });

      missions.splice(0, missions.length, ...finalMissions);
  ...
  async function loadFallbackScript() {
    return new Promise((resolve) => {
      ...
      script.src = "js/listening-data-fallback.js?v=20260731unifiedtone";
  ...
  async function loadListeningMissionsFromApi() {
    try {
      ...
      loaded.forEach((m) => {
        ...
        seenIds.add(m.id);
        const goalKey = m.goal || defaultGoal;
        if (!groupedByGoal[goalKey]) groupedByGoal[goalKey] = [];
        if (groupedByGoal[goalKey].length < 6) {
          groupedByGoal[goalKey].push(m);
        }
      });
  ...
  function createMissionFromSpec(spec, number) {
    const goal = spec.goal || defaultGoal;
    const mission = { 
      ...spec,
      goal: goal,
      tone: getToneFromGoal(goal)
    };
  ...
  window.initListeningLab = async function initListeningLab() {
    ...
    function getTopicSessionLabel(mission) {
      if (mission && typeof mission.sessionOrder === "number") {
        return `Session ${mission.sessionOrder}`;
      }
